...
// ✅ 서버 액션(생성 AI만 수행) — 기존과 동일 사용
import { generateIdea, type GeneratedIdea } from '@/app/actions';

// ✅ 사용량 조회는 클라이언트에서만 (서버에서 Firestore 만지면 규칙에 막힘)
import { getUserUsageClient } from '@/lib/firebase-client';
...
export default function IdeaHero() {
  ...
  const { user, loading: authLoading } = useAuth(); // ✅ 인증 로딩 상태까지 가져온다
  ...
  // 로그인 상태 변화 시 사용량 불러오기 (인증 로딩 끝 + uid 있을 때만)
  const uid = user?.uid ?? null;
  const loadingUsageRef = useRef(false);

  useEffect(() => {
    // ✅ 1) 인증 로딩 동안에는 아무 것도 하지 않음 (레이스 차단)
    if (authLoading) return;

    // ✅ 2) 인증 확정됐는데 유저 없으면 상태 리셋
    if (!uid) {
      setRole(null);
      setDailyLeft(null);
      setIdeasLeft(null);
    ...
    (async () => {
      try {
        ...
        const res = await getUserUsageClient(uid); // ✅ 클라이언트 함수
    ...
    return () => { alive = false; };
  }, [authLoading, uid]); // ✅ 인증 로딩 상태까지 의존

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    // ✅ 인증 로딩 중이거나 사용자 없으면 액션 차단
    if (authLoading || !user) {
      e.preventDefault();
      if (!authLoading) {
            ...
            <form ref={formRef} action={formAction} onSubmit={handleSubmit} className="relative" aria-busy={pending}>
              <div className="relative">
                <input
                  ...
                  disabled={authLoading || pending || (role === 'free' && (dailyLeft === 0 || (ideasLeft !== null && ideasLeft === 0)))}
                  className={cn(
                    'w-full px-6 py-5 pr-20 border rounded-2xl shadow-lg text-lg transition-all duration-200',
                    'focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent',
                    'group-hover:shadow-xl',
                    (authLoading || pending) ? 'bg-gray-50 cursor-not-allowed' : 'bg-white',
                    (role === 'free' && (dailyLeft === 0 || (ideasLeft !== null && ideasLeft === 0)))
                ...
                <Button
                  type="submit"
                  disabled={authLoading || pending || !idea.trim() || (role === 'free' && (dailyLeft === 0 || (ideasLeft !== null && ideasLeft === 0)))}
                  className={cn(
